...
//优雅的错误输出
aries.prettyStackFunc = function(err){
	if(!err.stack){
		return err;
	}else{
...
aries.compile = function(tempstr, ctx, cb){
		...
		aries.scanInclude(tempStrList, 0, ctx, function(err, strList, noInclude, includeIds, includeFiles){
							
				//避免回调2次
				if(compileFinish) return;
				if(err){
					compileFinish = true
					err = aries.prettyStackFunc(err);
					return cb(err);
				}
				...
				var templatePromise = te.template(strList, key, function (err, t, isUseCache) {
						...
						templateResponse.addListener("complete", function () {
								...
								if(ctx._ariestpError){
									err = aries.prettyStackFunc(ctx._ariestpError);
...
aries.assemble = function assemble(tempStrList, includeList, notHaveInclude, ctx, cb){
	...
	cb(null, tempStrList, notHaveInclude, cb.includeIds, cb.includeFiles);	
}
